...
function LogIn() {
  ...
  const handleSecurityQuestionSubmit = async (e) => {
    e.preventDefault();
    dispatch({type:"SetForgotEmail", payload:{forgotEmail: e.target.email.value}});
    try {
      const response = await fetch("http://localhost:5500/forgotPassword/compareSecurityAnswer", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          email: e.target.email.value,
      ...
      if (result.success) {
        ...
        console.log("Security answer matched! Redirecting to reset password page.");
